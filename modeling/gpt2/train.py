...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # be the batch dim
        # nh is "number of heads", hs is "head size", and C (num channels) = nh * hs
        # e.g. in GPT-2 (124M), n_head=12, hs=64, so nh*hs=C=768 channels in the
        # Transformer
        qkv = self.c_attn(x)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type: str):
        ...
        # copy while ensuring all of the parameters are aligned and match in names and
        # shapes
